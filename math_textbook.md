...
**A confound, found and closed: unseeded $k$-means.** `kmeans_gpu`'s Forgy initialization (§I.7, used whenever `--k-clusters` is a fixed integer rather than `auto`) drew its starting centroids from `torch.randperm` on PyTorch's *global* RNG, with no seed set anywhere in the pipeline. Every invocation therefore landed in a different local optimum, independent of the VPS features fed in — so a first smoke-scale comparison of just $L^2$ vs. cosine ($D_f=1.669$ vs. $1.486$, a large gap) and a second run adding the rest of the $p$-norm family ($1.669$–$1.675$ across six norms, cosine at $1.685$ — all within a band of $0.016$) looked like two different experiments *disagreeing*, when really they were two unseeded $k$-means fits that could never have been compared to begin with. `kmeans_gpu` now takes a `seed` (default $42$, matching `select_optimal_clusters`'s existing `random_state=42`) applied via a local `torch.Generator` that does not touch global RNG state elsewhere in the pipeline; two independent process invocations of the identical smoke configuration now return bit-identical $D_f$ for both `l2` ($1.671$, unchanged from before within rounding) and `cosine` ($1.486$, exactly reproducing the *first* run, not the second).

**Preliminary finding (now reproducible).** With the confound closed, $L^2$ and cosine give a real, deterministic $D_f$ gap of $\approx0.185$ at smoke scale ($1.671$ vs. $1.486$, grid $24^2$, $K=0.5$, $t_{\max}=10$) — this is evidence the norm choice can matter, not an artifact, though still far below production length. The clustering-free lobe-sign label (I.28) was, as it must be, identical across every norm tested ($\gamma_{\text{sign}}=0.311$ throughout), confirming the swap touches only the VPS/$k$-means path of §I.7 and not the independent labeling of §I.8. The actual experiment — every value on the same $K$, node pair, and production-resolution grid, now with a seeded and therefore trustworthy clustering step — is still to run.
