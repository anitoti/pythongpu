...
plt.tight_layout()
plt.savefig(SAVE + 'basin_map_continuous.png', bbox_inches='tight')
plt.close()
...
#
# Here KmeansMat = R.ravel() reshaped as a column vector —
# each IC gets one scalar feature (its final R value).
# For richer clustering, swap in the full final theta (B×N).

R_flat  = data['theta']           # (m², 1) — feature per IC
...
for k in range(K_clus):
    ...
    ax.scatter(Xg[mask], Yg[mask],
               color=colors[k],
               s=40,                        # MarkerSize=1.75 equivalent
...
plt.tight_layout()
plt.savefig(SAVE + 'basin_map_kmeans.png', bbox_inches='tight')
plt.close()
