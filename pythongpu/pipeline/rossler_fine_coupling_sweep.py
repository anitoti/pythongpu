#!/usr/bin/env python3
"""
Fine coupling sweep of the DTI-coupled Rössler basin geometry.

Starting point for bringing Rössler into the multi-system comparison
alongside Lorenz and Hindmarsh-Rose. STREAMING SURROGATE ONLY -- no true-VPS
support yet (unlike hr_fine_coupling_sweep.py / lorenz_fine_coupling_sweep.py)
and the lobe-locking sign(mean_x) label is unvalidated for this system (see
rossler_sweep.py's run_sweep_streaming docstring: the coupling-induced
IC-spread signal was found "inconclusive" here, not confirmed like Lorenz's).

IMPORTANT: Rössler's useful coupling window is much smaller than Lorenz/HR's.
RosslerParams' own docstring: "the X-coupled Rössler destabilises above
~0.15 (Z→∞)", so the default k-start/k-stop below stay under that ceiling,
unlike Lorenz/HR's [0.45, 0.65] window. Also uses RosslerParams' own smaller
dt=0.01 (the DTI-coupled Rössler is stiffer, eig_max(L)≈45).

Usage
-----
    python3 -m pythongpu.pipeline.rossler_fine_coupling_sweep
    python3 -m pythongpu.pipeline.rossler_fine_coupling_sweep \
        --node-x 28 --node-y 79 --k-start 0.02 --k-stop 0.10 --k-step 0.02 \
        --grid-n 128 --outdir data/derivatives
    python3 -m pythongpu.pipeline.rossler_fine_coupling_sweep --smoke
"""
...
from pythongpu.pipeline.rossler_sweep import (
    RosslerParams,
    rk4_step_batched,
    run_sweep_streaming,
)
...
# ── slice construction ───────────────────────────────────────
def build_slice(p: RosslerParams, grid_n: int, grid_lo: float, grid_hi: float,
                device: torch.device):
    """
...
# ── one coupling value ───────────────────────────────────────
def observe_coupling(L_gpu, coupling: float, p_base: RosslerParams, grid_n: int,
                     grid_lo: float, grid_hi: float, k_clusters,
                     cluster_criterion: str, device: torch.device,
                     kmeans_seed: int = 42) -> dict:
    """Integrate, cluster, and quantify the basin geometry for one K.

    Streaming surrogate only -- see module docstring for what's not here yet.
    """
    p = RosslerParams(
        a=p_base.a, b=p_base.b, c=p_base.c,
    ...
    vectors_gpu, mean_x_gpu = run_sweep_streaming(x0, L_gpu, p, device, return_mean_x=True)
    vectors = vectors_gpu.cpu().numpy()

    # ── sign(mean_x) label field -- UNVALIDATED for Rössler, see docstring ──
    ...
    if str(k_clusters).lower() == "auto":
        ...
        from pythongpu.pipeline.rossler_sweep import kmeans_gpu
...
def npz_name(node_x: int, node_y: int, coupling: float) -> str:
    return f"rossler_basins_n{node_x}_n{node_y}_K{coupling:.4f}.npz"


def gif_name(node_x: int, node_y: int, k_start: float, k_stop: float) -> str:
    return f"rossler_basins_n{node_x}_n{node_y}_K{k_start:g}-{k_stop:g}.gif"
...
def render_frame(rec: dict, node_x: int, node_y: int, lbl_x: str, lbl_y: str,
    ...
    ax.set_title(
        f"DTI_A Rössler basins   coupling K = {rec['coupling']:.4f}\n"
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--node-x", type=int, default=28, help="grid x-axis node")
    ap.add_argument("--node-y", type=int, default=79, help="grid y-axis node")
    ap.add_argument("--k-start", type=float, default=0.02,
                    help="Rössler destabilises above ~0.15 (Z->inf) -- default ladder "
                         "stays well under that, unlike Lorenz/HR's [0.45,0.65].")
    ap.add_argument("--k-stop", type=float, default=0.10)
    ap.add_argument("--k-step", type=float, default=0.02)
    ...
    p_base = RosslerParams(slice_node_x=args.node_x, slice_node_y=args.node_y,
                           n_osc=n_dti)
    grid_n = args.grid_n
    if args.smoke:
        grid_n = 24
        p_base = RosslerParams(coupling=0.05, dt=0.01, t_transient=5.0, tmax=5.0,
    ...
    for K in ladder:
        ...
        cfg = dict(coupling=rec["coupling"], slice_node_x=args.node_x,
                   ...
                   dt=p_base.dt, tmax=p_base.tmax, kmeans_seed=args.kmeans_seed)
