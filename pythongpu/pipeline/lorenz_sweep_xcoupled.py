#!/usr/bin/env python3
"""
filename: lorenz_vps_clustering_xcoupled.py
------------------------
Author : anitoti
Date   : summer 2026
...
COUPLING: X-component only.
  H = [0 0 0; 0 1 0; 0 0 0] — selects X (index 0) from each oscillator.
...
def lorenz_rhs_batched(
    ...
    """
    Batched RHS — DTI Laplacian coupling on X (index 0) only.
    ...
    # Coupling applied to X component only — index 0
...
def run_sweep_streaming(
    ...
    """
    Welford online accumulation of VPS features.
    [Page 37, full_.m_script.pdf:
     "KmeansMat = zeros(NumCond, 2*size(Vec,1))"]
    """
    B, N, _ = x0.shape
    ...
    for _ in tqdm(range(p.steps_record), desc="streaming"):
        ...
        dx_abs = diff[..., 0].abs()               # (B, C) — X component
...
def main():
    ...
    ap = argparse.ArgumentParser(
        description="DTI-coupled Lorenz basin map — X coupling"
    )
    ...
    ax_el.set_title("Elbow Method — X-coupled")
    ax_el.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(get_plot_path("lorenz_vps_clustering_xcoupled", f"elbow_curve_{timestamp}.png", args.outdir), dpi=150)
    plt.close(fig_el)
    ...
    ax_bm.set_title(
        f"K-Means Basin Map — X-coupled (k={args.k_clusters})\n"
        f"N={p.n_osc}  coupling={p.coupling}  grid={m}²"
    )
    plt.colorbar(im, ax=ax_bm, label="Basin label")
    plt.tight_layout()
    plt.savefig(get_plot_path("lorenz_vps_clustering_xcoupled", f"basin_map_kmeans_{timestamp}.png", args.outdir), dpi=150)
    plt.close(fig_bm)
    ...
    ax_bd.set_title(
        f"Basin Boundary — X-coupled\n"
        f"N={p.n_osc}  coupling={p.coupling}  grid={m}²"
    )
    plt.tight_layout()
    plt.savefig(get_plot_path("lorenz_vps_clustering_xcoupled", f"basin_boundary_{timestamp}.png", args.outdir), dpi=150)
    plt.close(fig_bd)
    ...
    ax_bc.set_title("Box-Counting — X-coupled")
    ax_bc.legend()
    ax_bc.grid(True, which="both", alpha=0.3)
    plt.tight_layout()
    plt.savefig(get_plot_path("lorenz_vps_clustering_xcoupled", f"boxcount_loglog_{timestamp}.png", args.outdir), dpi=150)
    plt.close(fig_bc)
    ...
    ax_bx.axis("off")
    plt.tight_layout()
    plt.savefig(get_plot_path("lorenz_vps_clustering_xcoupled", "boxdiv2_synthetic.png", args.outdir), dpi=150)
    plt.close(fig_bx)
    ...
    for ax in axes:
        ax.axis("off")
    plt.tight_layout()
    plt.savefig(get_plot_path("lorenz_vps_clustering_xcoupled", "rewired_adjacency.png", args.outdir), dpi=150)
    plt.close(fig_rw)
    print("[saved]    rewired_adjacency.png")

    # ── Save — FIX: use coupling-tagged filename ─────────────────────────
