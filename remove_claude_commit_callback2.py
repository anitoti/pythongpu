...
def commit_callback(commit):
    if getattr(commit, 'author_name', None) is not None:
        name = normalize_and_strip_claude(decode_bytes(commit.author_name))
        commit.author_name = encode_str(name)

    if getattr(commit, 'committer_name', None) is not None:
        name = normalize_and_strip_claude(decode_bytes(commit.committer_name))
        commit.committer_name = encode_str(name)

    if getattr(commit, 'author_email', None) is not None:
        email = normalize_and_strip_claude(decode_bytes(commit.author_email))
        commit.author_email = encode_str(email)

    if getattr(commit, 'committer_email', None) is not None:
        email = normalize_and_strip_claude(decode_bytes(commit.committer_email))
        commit.committer_email = encode_str(email)

    if getattr(commit, 'message', None) is not None:
        message = decode_bytes(commit.message)
        message = remove_coauthored_by_trailers(message)
        message = normalize_and_strip_claude(message)
        commit.message = encode_str(message)
